...
//-----------------------------------------------------------------------------
class C_RagePickup : public C_BaseAnimating
{
	DECLARE_CLASS(C_RagePickup, C_BaseAnimating);
	DECLARE_CLIENTCLASS();
public:
	C_RagePickup() {
...
LINK_ENTITY_TO_CLASS(item_rage, C_RagePickup);

IMPLEMENT_CLIENTCLASS_DT(C_RagePickup, DT_RagePickup, CRagePickup)
RecvPropBool(RECVINFO(m_bRespawning)),
END_RECV_TABLE()

inline bool C_RagePickup::IsRespawning()
{
	return m_bRespawning;
}

void C_RagePickup::ClientThink()
{
...
void C_RagePickup::PostDataUpdate(DataUpdateType_t updateType)
{
